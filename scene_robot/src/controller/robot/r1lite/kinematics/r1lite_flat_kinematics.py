...
from controller.robot import CONTROLLER_RESOURCE_DIR
from scipy.spatial.transform import Rotation as R

class R1LiteFlatKinematics(RobotKinematics):
    def __init__(self, robot_cfg: RobotConfig, **kwargs) -> None:
        super().__init__(robot_cfg)
        self.robot_cfg = robot_cfg
        self.mixed_jointsToLockIDs = self.robot_cfg.joint_to_lock
        self.kinematics_model_path = 'r1lite/r1lite_upper_body.xml'
    ...
    def _init_fixed_base_kinematics(self):
        self.fixed_base_robot = pin.RobotWrapper.BuildFromMJCF(
            os.path.join(CONTROLLER_RESOURCE_DIR, self.kinematics_model_path)
        )
        ...
        self.arm_first_joint_indices = np.array([3, 9], dtype=int)
        self.arm_first_joint_lower_bound = np.array([-1.0, -1.0], dtype=float)
        self.arm_first_joint_upper_bound = np.array([1.0, 1.0], dtype=float)
        ...
        self.cTf_p = casadi.SX.sym("tf_p", 4, 4)
        ...
        self.translational_error = casadi.Function(
            "translational_error",
            [self.cq, self.cTf_l, self.cTf_r,self.cTf_p],
            [
                casadi.vertcat(
                    self.cdata.oMf[self.L_hand_id].translation - self.cTf_l[:3,3],
                    self.cdata.oMf[self.R_hand_id].translation - self.cTf_r[:3,3],
                    self.cdata.oMf[self.platform_id].translation[:2] - self.cTf_p[:2,3],
                )
            ],
        )
        def _skew_rot_error(cur_rot, target_rot):
            # More numerically robust than log3 around singular orientations.
            r_err = cur_rot @ target_rot.T
            return 0.5 * casadi.vertcat(
                r_err[2, 1] - r_err[1, 2],
                r_err[0, 2] - r_err[2, 0],
                r_err[1, 0] - r_err[0, 1],
            )
        ...
        self.rotational_error = casadi.Function(
            ...
            [
                casadi.vertcat(
                    _skew_rot_error(self.cdata.oMf[self.L_hand_id].rotation, self.cTf_l[:3, :3]),
                    _skew_rot_error(self.cdata.oMf[self.R_hand_id].rotation, self.cTf_r[:3, :3]),
                    _skew_rot_error(self.cdata.oMf[self.platform_id].rotation, self.cTf_p[:3, :3]),
                )
            ],
        )
        self.platform_rotational_error = casadi.Function(
            "platform_rotational_error",
            [self.cq, self.cTf_p],
            [_skew_rot_error(self.cdata.oMf[self.platform_id].rotation, self.cTf_p[:3, :3])],
        )
        ...
        self.param_tf_p = self.opti.parameter(4, 4)
        self.translational_cost = casadi.sumsqr(self.translational_error(self.var_q, self.param_tf_l, self.param_tf_r,self.param_tf_p))
        self.rotation_cost = casadi.sumsqr(self.rotational_error(self.var_q, self.param_tf_l, self.param_tf_r,self.param_tf_p))
        self.platform_rotation_cost = casadi.sumsqr(
            self.platform_rotational_error(self.var_q, self.param_tf_p)
        )
        ...
        self.opti.subject_to(
            self.opti.bounded(
                self.arm_first_joint_lower_bound,
                self.var_q[self.arm_first_joint_indices],
                self.arm_first_joint_upper_bound,
            )
        )
        ...
        self.opti.minimize(
            ...
            + 10 * self.platform_rotation_cost
    ...
    def add_extra_frames(self, model):
        ...
        for link_name, spheres in collision.items():
            ...
            if link_name not in link_to_joint:
                raise RuntimeError(
                    f"No joint mapping for collision link '{link_name}'"
                )
    ...
    def inverse_kinematics(self, T , current_lr_arm_motor_q = None, current_lr_arm_motor_dq = None):
        ...
        self.opti.set_value(self.param_tf_p, np.eye(4))
