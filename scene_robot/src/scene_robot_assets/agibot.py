...
* :obj:`AGIBOT_G1_OMNIPICKER_CFG`: Local Agibot G1 omnipicker robot
...
def _resolve_local_agibot_usd(*relative_candidates: str) -> str:
    """Resolve an existing local Agibot USD/USDA asset path.

    The repository contains multiple converted Agibot assets. We resolve against the
    files that actually exist in this workspace instead of assuming a single fixed path.
    """

    robot_root = os.path.join(os.path.dirname(__file__), "agibot")
    for relative_path in relative_candidates:
        usd_path = os.path.join(robot_root, relative_path)
        if os.path.exists(usd_path):
            return usd_path
    raise FileNotFoundError(
        "Unable to resolve Agibot USD asset. Tried: "
        + ", ".join(os.path.join(robot_root, path) for path in relative_candidates)
    )
...
_AGIBOT_G1_OMNIPICKER_USD_PATH = _resolve_local_agibot_usd(
    "G1_omnipicker/robot.usda",
    "G1_omnipicker/robot.usd",
)

AGIBOT_G1_OMNIPICKER_CFG = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        usd_path=_AGIBOT_G1_OMNIPICKER_USD_PATH,
        activate_contact_sensors=True,
        rigid_props=sim_utils.RigidBodyPropertiesCfg(
            disable_gravity=False,
            max_depenetration_velocity=5.0,
    ...
    # For this locally converted USD, keep actuator gains aligned with conversion defaults (stiffness=100, damping=1).
    actuators={
        "body": ImplicitActuatorCfg(
            joint_names_expr=["idx01_body_joint1", "idx02_body_joint2"],
...
AGIBOT_G1_OMNIPICKER_BIMANUAL_CFG = AGIBOT_G1_OMNIPICKER_CFG.copy()
AGIBOT_G1_OMNIPICKER_BIMANUAL_CFG.spawn.rigid_props.disable_gravity = True
AGIBOT_G1_OMNIPICKER_BIMANUAL_CFG.actuators = {
    "body": ImplicitActuatorCfg(
        joint_names_expr=["idx01_body_joint1", "idx02_body_joint2"],
        effort_limit_sim=None,
        velocity_limit_sim=None,
...
"""Configuration of Agibot G1 Omnipicker for bimanual manipulation."""


AGIBOT_G1_OMNIPICKER_STACK_CUBE_DIFF_IK_CFG = AGIBOT_G1_OMNIPICKER_BIMANUAL_CFG.copy()
AGIBOT_G1_OMNIPICKER_STACK_CUBE_DIFF_IK_CFG.spawn.articulation_props.fix_root_link = True
"""Configuration of Agibot G1 Omnipicker tuned for stack-cube Differential IK demos."""
