...
class AgentServiceTest(unittest.TestCase):
    ...
    def test_planner_executor_runs_edit_then_generate(self) -> None:
        context = self._create_context()
        context.scene_graph_path.write_text(json.dumps(REAL2SIM_SCENE_GRAPH), encoding="utf-8")
        context.render_path.write_bytes(b"png")
        context.default_placements_path.write_text(
        ...
        self.assertEqual(result["agent"]["state"], "completed")
        self.assertEqual(result["agent"]["completed_state"], "generate_scene")
        self.assertEqual(result["execution_plan"]["executed_steps"], ["edit_scene_graph", "generate_scene"])
        edit_mock.assert_called_once()
        run_scene_mock.assert_called_once()

    def test_planner_executor_persists_plan_while_waiting_for_layout_strategy(self) -> None:
        context = self._create_context()
        context.scene_graph_path.write_text(json.dumps(REAL2SIM_SCENE_GRAPH), encoding="utf-8")
        ...
        with (
            ...
            mock.patch.object(agent_service, "apply_instruction", return_value=edit_result),
        ):
            result = agent_service.handle_agent_message(
                session_id=context.session_id,
                run_id=context.run_id,
        ...
        self.assertEqual(result["agent"]["state"], "await_layout_strategy")
        self.assertEqual(result["execution_plan"]["executed_steps"], ["edit_scene_graph"])
        state_payload = json.loads((context.session_root / "agent_state.json").read_text(encoding="utf-8"))
        pending_plan = state_payload["pending_plan"]
        self.assertEqual(pending_plan["current_step_index"], 1)
        self.assertEqual(pending_plan["warnings"], ["placement adjusted"])
